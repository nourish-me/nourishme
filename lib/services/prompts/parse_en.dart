...
- DHA: ALWAYS estimate dha_mg whenever a real DHA source is in the meal, using these anchors, and do NOT shrink the values: fatty sea fish (salmon 1100-1400, herring 1500-2000, mackerel 1100-1300, sardine 900-1100 mg/100 g - so a 150 g salmon fillet is well over 1000 mg, not a few hundred), egg yolk 30-40 mg per whole egg (egg white alone near zero), fish oil / algae oil supplement per its label. A chicken egg in ANY form (boiled, scrambled, fried, poached, in a dish) is a real DHA source and ALWAYS counts ~35 mg DHA per egg, never omit it. Only OMIT dha_mg entirely when the meal contains NONE of these sources, i.e. lean meat, lean fish, and purely plant-based meals. Plant omega-3 sources (flaxseed, chia, walnuts, almonds, rapeseed oil, soy oil) contain ALA (precursor), NOT DHA - the body converts under 5% of ALA to DHA and we do NOT count that as DHA.
...
IMPORTANT: actively estimate the micronutrients for EVERY recognizable food from standard nutritional values. Use the anchors above as guidance even when the exact food is not named there (e.g. oats, quinoa, millet, amaranth carry iron like other whole grains; treat them so). Only skip a key when this meal's contribution is genuinely below ~5% of the daily reference (DGE 2025) - do NOT "omit when in doubt", omit only on a true sub-5%. Conversely, do not invent nutrients the food does not contain (no DHA for plants, no B12 for purely plant-based meals). For meals with no notable micronutrients (e.g. water, plain sugar drink) omit the entire micronutrients field. Values are PER THIS MEAL, not per 100 g.
