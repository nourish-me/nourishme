...
- [ ] **Quick-reply chips suggest fish to vegetarians** · Lotte (1) · #P2 · 🔧 fix v2, needs re-verify · [[beta-feedback-log#2026-06-21 · Lotte (T11) · current beta · WhatsApp|→ Log]]
	Lotte set Diet = Vegetarian but a coach quick-reply offered fish. The chips are freely generated by the model, so the literal-example swap wasn't enough: a vegetarian device test (Vanessa, 2026-06-22) still produced "Ich esse wenig Fisch und Eier". Fix v2 in claude_client.dart tightens BOTH locales: the reply templates MUST fit the user's diet and must never mention a food she doesn't eat at all (no fish/meat for vegetarian/vegan, no eggs/dairy for vegan). analyze clean, 330 tests green. Re-verify on device: a vegetarian profile gets no fish/meat chip.
