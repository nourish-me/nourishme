...
- [ ] **Supplement-Label-Scan erkennt/speichert Werte nicht zuverlässig** · Julia (1) · #P2 · [[beta-feedback-log#2026-06-21 · Julia (T10) · current beta · E-Mail|→ Log]]
	Julia photographed her Fetesept package (with the Nährwerttabelle) at setup, but the coach couldn't link her supplement until she told it explicitly, twice. Code path verified: `parseSupplementLabel` → review sheet pre-fills the value fields from the parsed values; on save only fields > 0 are stored (`supplement_setup.dart:261`). So if the vision parse misses the nutrient table, the supplement saves NAME-ONLY (no values) and the coach has no amounts to link, independent of the coach-context fix. CORRECTED from initial triage: this is NOT the coach-context gap, it's supplement-scan parse/save quality (Vanessa's read). OPEN: investigate the scan→save path (why values were dropped / parse robustness), and/or an investigative question to Julia (did the review sheet show values after the photo, or were the fields blank?).
