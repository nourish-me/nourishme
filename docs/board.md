...
- [ ] **Parser unterschätzt Mikros bei nicht-verankerten Lebensmitteln (Prompt-Bias + Golden-Corpus)** · Rebecca + Henrike (2) · #P1 #parse
	The parse prompt is written defensively ("im Zweifel den Key weglassen") and leans on named food anchors, so the model systematically UNDER-reports micronutrients for any food not explicitly listed. Two confirmed instances, each first patched one-off: DHA omitted for eggs (egg yolk wasn't in the DHA zero-rule's allowlist, fixed 2026-06-22) and iron omitted for oats (Haferflocken not a named iron anchor). The worker confirms the model KNOWS the value once permitted (1 egg → 35 mg DHA), so it's prompt suppression, not missing knowledge. Adding anchors per food is whack-a-mole (next: Quinoa, then fish...). Needs a design pass (Explore), sibling to "Scalable safety matching". Options: (1) flip the default bias to "estimate from standard nutritional values for ALL plausible foods, omit only when genuinely <5% of the daily reference"; (2) a golden food→micros regression corpus that catches both under- AND over-reporting before testers do. Lean (1) guarded by (2). Risk: loosening can cause over-reporting/hallucination, which the corpus must catch. ENTSCHIEDEN (Vanessa, 2026-06-22): KEIN Stopgap-Patch, direkt strukturell rangehen. Heißt: kein einzelner Hafer/Quinoa-Anker-Patch, der Eisen-Fall (Henrike) wird über die strukturelle Lösung mitgefixt. Nächster Schritt: /explore auf diese Karte (Prompt-Bias-Audit + Golden-Corpus-Design).
...
- [ ] **Confirm-Screen: Datum-Chip wirkt wie Pflicht-Schritt + wilde Reihenfolge beim Nachtragen** · Lotte (1) · #P2 · ⏳ Rückfrage an Lotte gesendet 2026-06-22 · [[beta-feedback-log#2026-06-21 · Lotte (T11) · current beta · WhatsApp|→ Log]]
	Two intertwined points. (1) The today-date chip in the confirm screen feels like a mandatory confirmation step even though it blocks nothing. Planned fix: make the date non-interactive-looking for "today" (retro entries keep it visible/tappable). (2) Backfilled entries land in wild order: verified the thread sorts correctly on `createdAt`, so the cause is that the time chip isn't adjusted when backfilling and all entries get the save-time. Likely a consequence of (1). Asked Lotte investigatively whether the planned date fix helps and what exactly happened with the times (+ screenshot). Holding BOTH until her answer, don't build the date fix or a separate ordering fix before she replies.
