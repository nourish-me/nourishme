...
- [ ] **Tagebuch-Einträge nicht chronologisch trotz korrekt gesetzter Zeit** · Lotte (1) · #P2 #bug · ❓ Repro offen · [[beta-feedback-log#2026-06-21 · Lotte (T11) · current beta · WhatsApp|→ Log]]
	Lotte hat gestern Abend Mahlzeiten für den Tag nachgetragen UND die Uhrzeiten korrekt gesetzt (Video 2026-06-22), aber im Tagebuch stehen die Einträge nicht chronologisch nach Zeit. WIDERSPRUCH zum Code: jedes Meal-ThreadItem wird mit `at: meal.createdAt` angelegt (confirm_screen.dart:537, home_input.dart:773, home_screen.dart:950) und thread_repository.getForDate sortiert per mergeSort genau auf diese timestamp, also MÜSSTE die Chip-Zeit = Sortier-Zeit sein. Stichprobe der Video-Frames (16:39 Radler/Butterzopf über 17:40 Pizza) sah sogar korrekt sortiert aus. Prime-Verdacht für „Chip richtig, Reihenfolge falsch": ein Zeit-Setz-/Edit-Pfad, der MealEntry.createdAt (→ Chip) aktualisiert, aber das ThreadItem.timestamp (→ Sort) NICHT nachzieht (thread_repository.updateMealTime existiert, wird der Backfill-/Confirm-Pfad da durchgeleitet?). ❓ Offen: welche zwei Einträge im Video stehen konkret falsch (Zeiten)? Das nagelt die Repro. Verwandt: „Bundled scan + text ordering", „ThreadRepository.add() race". (Ich hatte das fälschlich als gelöst nach Shipped geschoben, zurückgeholt.)
